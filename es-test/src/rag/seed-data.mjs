/**
 * ES + Milvus 种子数据。插入数据 分别在es 和milvues中插入
 */
...
async function seedMilvus(collectionName, rows, emb) {
  try {
    ...
    console.log(
      `提示: Attu 请连接 ${MILVUS_ADDRESS}，数据库选 default，集合名「${collectionName}」。若 Attu 跑在 Docker 里，请用 host.docker.internal 或宿主机 IP，勿用容器内的 localhost。`,
    );
  } catch (error) {
