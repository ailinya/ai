...
// 缓解 Windows 下 IPv6 优先但链路不通导致的 fetch failed；BOCHA_DNS_IPV4_FIRST=0 可关闭
//本文件增加了网络搜素和评估节点
if (process.env.BOCHA_DNS_IPV4_FIRST !== "0") {
  dns.setDefaultResultOrder("ipv4first");
}

// 与 advanced-rag/.env 一致：DashScope 用 DASHSCOPE_API_KEY；兼容仍使用 OPENAI_API_KEY 的环境
const dashscopeApiKey =
  process.env.DASHSCOPE_API_KEY || process.env.OPENAI_API_KEY;

if (!dashscopeApiKey) {
  console.error(
    "错误: 请设置 DASHSCOPE_API_KEY（或 OPENAI_API_KEY）及 OPENAI_BASE_URL。",
  );
  process.exit(1);
}
...
// Bocha 单次请求超时（毫秒）
const BOCHA_FETCH_TIMEOUT_MS =
  Number(process.env.BOCHA_FETCH_TIMEOUT_MS) || 45_000;
// 连接层失败时的重试次数（至少 1）
const BOCHA_FETCH_RETRIES = Math.max(
  1,
  Math.floor(Number(process.env.BOCHA_FETCH_RETRIES)) || 3,
);

/** 从环境变量读取 HTTP(S) 代理，供 undici ProxyAgent 使用 */
function getBochaProxyDispatcher() {
  const proxyUrl = (
...
/**
 * 调用 Bocha 联网搜索（undici + 可选代理 + 超时 + 重试）
 */
async function bochaWebSearch(query, count) {
  ...
  const dispatcher = getBochaProxyDispatcher();
  console.log(dispatcher);
  
  if (dispatcher && process.env.BOCHA_DEBUG_PROXY === "1") {
    console.log("【Bocha】使用 HTTPS_PROXY/HTTP_PROXY 访问 API");
  }

  let response;
  for (let attempt = 1; attempt <= BOCHA_FETCH_RETRIES; attempt++) {
    // 每次重试使用新的超时信号（timeout 只能触发一次）
    const signal =
      typeof AbortSignal !== "undefined" && AbortSignal.timeout
        ? AbortSignal.timeout(BOCHA_FETCH_TIMEOUT_MS)
        : undefined;
    try {
      ...
      const isFetchFailed =
        ...
        msg === "Failed to fetch" ||
        name === "TypeError";
      const retriable = isFetchFailed || isTimeout;
      if (!retriable || attempt >= BOCHA_FETCH_RETRIES) {
        const detail = isTimeout
          ? `请求超时（>${BOCHA_FETCH_TIMEOUT_MS}ms）`
          : msg;
        throw new Error(
          `搜索 API 请求失败（网络错误）：${detail}。可检查：${url}、HTTPS_PROXY、防火墙；单独一行执行：Test-NetConnection api.bochaai.com -Port 443`,
        );
      }
      const waitMs = 800 * attempt;
      console.warn(
        `【Bocha】第 ${attempt}/${BOCHA_FETCH_RETRIES} 次请求失败，${waitMs}ms 后重试：${msg}`,
      );
      await sleep(waitMs);
    }
  }
  ...
  if (!response) {
    throw new Error("搜索 API 请求失败：未获得响应。");
  }
...
const webSearchNode = async (state) => {
  ...
  try {
    ...
    // 联网失败不中断整条图：占位上下文，后续仍可依本地 Milvus 生成
    const msg = error?.message ?? String(error);
    console.warn(`联网搜索失败: ${msg}`);
    webContext = `【联网搜索不可用】${msg}\n请模型仅依据用户问题与本地知识库上下文作答；涉及剧集集数、外链等若无法核实请明确说明。`;
  }
